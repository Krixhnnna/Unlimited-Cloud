// app.js - Complete working implementation with new features
class TGDriveApp {
    constructor() {
        ...
        this.selectedFiles = new Set(); // NEW: For multi-select
        this.currentView = 'home'; // NEW: Track current view (home, starred, bin)
    ...
    bindBulkActions() {
        ...
        if (bulkCopyBtn) {
            bulkCopyBtn.addEventListener('click', () => this.showCopyMoveModal('copy'));
        }
        
        if (bulkMoveBtn) {
            bulkMoveBtn.addEventListener('click', () => this.showCopyMoveModal('move'));
        }
    ...
    showCopyMoveModal(action) {
        // For now, just show an alert - you can implement a proper modal
        alert(`${action} functionality will be implemented with a folder selection modal`);
    }
    
    // NEW: Star functionality
    async toggleFileStar(fileId) {
        try {
            ...
            if (response.ok) {
                ...
                console.log(`File ${fileId} star toggled:`, result.starred);
                
                // Update the file in current view
    ...
    async loadStarredFiles() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                throw new Error('Failed to load starred files');
            }
        } catch (error) {
    ...
    async loadBinFiles() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                throw new Error('Failed to load bin files');
            }
        } catch (error) {
        ...
        return `
            <div class="file-item theme-bg-secondary rounded-xl theme-border border p-4 hover:shadow-lg transition-all ${isMedia ? 'cursor-pointer' : ''}" 
                ...
                <div class="flex justify-between items-center">
                    ...
                    ${this.currentView === 'bin' ? `
                        <button class="restore-file-btn text-green-500 hover:text-green-700 p-2 rounded-lg hover:bg-green-50 transition-colors" data-file-id="${file.id}" title="Restore file">
    ...
    bindItemEvents() {
        ...
        // NEW: Restore file buttons (for bin view)
